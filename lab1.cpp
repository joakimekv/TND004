...
std::vector<int>::iterator TND004::stable_partition(std::vector<int>::iterator first, std::vector<int>::iterator last, std::function<bool(int)> p) {
	...
	// Recursive
		...
		std::vector<int>::iterator mid = first + (last - first) / 2;         // T(n) = O(1)

		// Left-hand side of the vector
		std::vector<int>::iterator it1 = stable_partition(first, mid, p);    // T(n) = O(log(n)), since n/2
